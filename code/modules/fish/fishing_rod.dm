...
// bait: list of what can be used as bait
...
/datum/bait_type/standard_bait
	bait = /obj/item/weapon/reagent_containers/food/snacks/shrimp
	fish = list(
		/obj/item/weapon/fish/goldfish = 1,
		/obj/item/weapon/fish/shark = 10,
		/obj/item/weapon/fish/electric_eel = 20,
		/obj/item/weapon/fish/glofish = 20,
		/obj/item/weapon/fish/catfish = 30,
		/obj/item/weapon/fish/salmon = 100,
		)

/datum/bait_type/fish_eggs
	bait = /obj/item/fish_eggs
	fish = list(
		/obj/item/fish_eggs/seadevil = 20
		)

/datum/bait_type/clown
	bait = /obj/item/clothing/mask/gas/clown_hat
	fish = list(
		/obj/item/weapon/bananapeel/clownfish = 100,
		)
...
/obj/item/weapon/fishingrod/suicide_act(mob/user)
	user.visible_message("<span class='danger'>[user] is whipping \his head open with \the [src]! It looks like \he took the bait!</span>")
...
/obj/item/weapon/fishingrod/afterattack(atom/A, mob/user, proximity_flag)
	...
	if(bait_fish)
		busy = TRUE
		to_chat(user, "<span class='notice'>You cast a line into \the [A_turf].</span>")
...
/obj/item/weapon/fishingrod/proc/do_fishing(var/mob/user, var/atom/target)
	...
	while(world.time < endtime)
		...
		if(rodtension >= MAX_LINE_TENSION)
			...
			break
		if(istype(hookeditem, /obj/item/weapon/fish)) // Only fish have a length and weight so we'll just use default values for non-fish items
			fish_pull()
		else
...
/obj/item/weapon/storage/bag/plasticbag/bait/New()
	..()
	for(var/i = 1 to 21)
		new /obj/item/weapon/reagent_containers/food/snacks/shrimp(src)